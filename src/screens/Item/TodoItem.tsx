    ...
    return (
        <button className='flex items-center justify-between w-full mb-4 text-xl rounded-2xl bg-gray-800 p-4'>
            <span className={cn({
                ...
                <button onClick={completeTodo}><Check isCompleted={todo.isCompleted} /></button>
                {editing ? <><EditTodoField todo={todo} /></> : <div>{todo.title}</div>}
            </span>
            <div>
                <button onClick={editTodo}><MdEdit className='text-green-500 hover:text-green-400 mx-3' size={23} /></button>
